...
VAEmodel = RNN_VAE(
    len(allTEXT.vocab), h_dim, z_dim, p_word_dropout=0.3,max_sent_len=30,
...
allmodel.load_state_dict(torch.load('SSTmodel/stanford-sentiment-treebank.train.glove300.classifier.bin'))
Beautymodel.load_state_dict(torch.load('SSTmodel/stanford-sentiment-treebank.train.fasttext300.classifier.bin'))
...
for it in range(100000):
    ...
    if it%1000==0:
        print("saving model stanford-sentiment-treebank.train_VAE.bin")
        print("\n")
        torch.save(VAEmodel.state_dict(), 'SSTmodel/{}.bin'.format('stanford-sentiment-treebank.train_VAE')) 
