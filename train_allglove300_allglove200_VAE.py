...
BeautyTEXT.build_vocab(Beautytrain, max_size=60000, vectors="glove.6B.200d",min_freq=1)
...
VAEmodel = RNN_VAE(
    len(allTEXT.vocab), h_dim, z_dim, p_word_dropout=0.3,max_sent_len=25,
...
VAEmodel.load_state_dict(torch.load('SSTmodel/allglove300_allglove200_VAE.bin'))
Beautymodel.load_state_dict(torch.load('SSTmodel/SSTtrainall_glove200.bin'))
...
for it in range(100000):
    ...
    if it%1000==0:
        print("saving model allglove300_allglove200_VAE.bin")
        print("\n")
        torch.save(VAEmodel.state_dict(), 'SSTmodel/{}.bin'.format('allglove300_allglove200_VAE')) 
